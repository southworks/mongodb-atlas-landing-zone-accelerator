...
module "observability" {
  ...
  create_blob_private_dns_zone      = true                         # Default: true (module creates the zone)
  blob_private_dns_zone_id          = null                         # Required when create_blob_private_dns_zone = false
}

# When another module already creates the blob private DNS zone (for example, the monitoring stack in the multi-region template),
# set create_blob_private_dns_zone = false and pass the existing zone ID through blob_private_dns_zone_id.
```
...
| create_blob_private_dns_zone    | Whether the module should create `privatelink.blob.core.windows.net` itself | bool   | no       | true    |
| blob_private_dns_zone_id        | Existing blob DNS zone ID to reuse (required when `create_blob_private_dns_zone = false`) | string | no       | null    |
...
- **Private Networking**:
  ...
  - Private DNS zones for each storage service endpoint (blob zone creation can be skipped by setting `create_blob_private_dns_zone = false` and passing an existing ID via `blob_private_dns_zone_id`)
  - Private DNS zone links to VNet for name resolution
